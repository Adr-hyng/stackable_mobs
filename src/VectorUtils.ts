...
export class Vec3 {
    ...
    /**
     * Creates a hash string from a Vector3 location for use as a constant identifier.
     * Useful for creating unique keys based on block/entity locations.
     * @param location The Vector3 location to hash.
     * @returns A string hash representing the location.
     */
    static hashCode(location: Vector3 | Block): string {
        let hash = 0;
        const locationArray = [location.x, location.y, location.z];
        for(let i = 0; i < 3; i++) {
            hash = (hash << 5) - hash + Number(locationArray[i].toFixed(2));
            hash = hash & hash; // Convert to 32-bit integer to avoid overflow
        }
