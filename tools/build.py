...
if not args.package_only:
    ...
    if args.watch:
        ...
        process_config_args = [sys.executable, 'tools/process_config.py', f'--target={watch_target}']
        if args.create_config_settings:
            process_config_args.append('--create-config-settings')
        subprocess.call(process_config_args, stdout=subprocess.DEVNULL)
        ...
        # Build settings file
        process_cfg_args = [sys.executable, 'tools/process_config.py', '-w', f'--target={watch_target}']
        if args.create_config_settings:
            process_cfg_args.append('--create-config-settings')
        process_cfg = subprocess.Popen(process_cfg_args, stdout=subprocess.DEVNULL)
        # Sync to com.mojang
    ...
    process_config_args = [sys.executable, 'tools/process_config.py', f'--target={args.target}']
    if args.create_config_settings:
        process_config_args.append('--create-config-settings')
    handleError(subprocess.call(process_config_args))
    if args.target == 'server':
        process_config_json_args = [sys.executable, 'tools/process_config.py', '--generateConfigJSON']
        if args.create_config_settings:
            process_config_json_args.append('--create-config-settings')
        handleError(subprocess.call(process_config_json_args))
